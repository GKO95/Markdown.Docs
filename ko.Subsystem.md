...
## 네이티브 이미지
[네이티브 이미지](https://en.wikipedia.org/wiki/Native_%28computing%29#Applications)(native image)는 어떠한 환경 서브시스템에도 종속되지 않은 실행 프로그램을 가리킨다. 다시 말해, [윈도우 서브시스템](#윈도우-서브시스템)의 `Kernel32.dll` 혹은 `User32.dll` 등을 필요하지 않으며 `Ntdll.dll`에서 제공하는 네이티브 API를 곧바로 호출한다. 네이티브 API는 대체로 문서화되어 있지 않으므로, 네이티브 이미지는 일반적으로 마이크로소프트에서 빌드한 프로그램이다.
