# 페이지 테이블
**[페이지 테이블](https://en.wikipedia.org/wiki/Page_table)**(page table)은 [가상 주소](Process.md#가상-주소-공간)와 [물리 주소](https://en.wikipedia.org/wiki/Physical_address) 간 매핑하는 [가상 메모리](Memory.md#가상-메모리) 체계에서 활용하는 데이터 구조이다.
