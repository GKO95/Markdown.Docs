...
* [환경 서브시스템](ko.Subsystem.md#환경-서브시스템)
* [윈도우 커널](ko.Kernel.md#윈도우-커널) 및 [장치 드라이버](ko.Driver.md#장치-드라이버)

## 시스템 서비스
[시스템 서비스](https://ko.wikipedia.org/wiki/시스템_호출)(system services)란, 운영체제 커널에서 제공하는 기능을 사용자 모드에서 호출할 수 있도록 하는 인터페이스이다. 사용자 모드 프로세스는 [보호 링](ko.Processor.md#보호-링) 구조에 의해 커널 함수(일명 [루틴](https://ko.wikipedia.org/wiki/함수_(컴퓨터_과학)); routine)을 직접 접근할 수 없으므로, [`Ntdll.dll`](#ntdlldll)에 내포된 진입점을 통해 [`Ntoskrnl.exe`](ko.Kernel.md)에 정의된 루틴을 호출하게 된다. 시스템 서비스를 호출한 프로세스의 [스레드](ko.Process.md#스레드)는 `syscall` 명령에 의해 사용자 모드에서 커널 모드로 전환되어, 제약에 걸렸던 모든 명령어들을 활용할 수 있게 된다. 그리고 커널 모드에서 작업이 완료되면 다시 사용자 모드로 돌아가는 원리로 동작한다.

### `Ntdll.dll`
[`Ntdll.dll`](https://ko.wikipedia.org/wiki/윈도우_라이브러리_파일#NTDLL.DLL)은 서브시스템 DLL 또는 네이티브 어플리케이션의 활용을 지원하는 특수 시스템 라이브러리이다.

> 윈도우에서 네이티브(native)란, 어떠한 [환경 서브시스템](ko.Subsystem.md#환경-서브시스템)에도 종속되지 않은 프로그램 이미지를 가리킨다. 세션 관리자 `Smss.exe`는 대표적인 네이티브 어플리케이션이며, 바로 환경 서브시스템의 도움없이 실행되어야 하기 때문이다.

`Ntdll.dll`이 지원하는 함수 유형은 다음과 같다:

1. [시스템 서비스](#시스템-서비스) 진입점
1. 일부 [C 런타임 라이브리러](ko.C.md) 루틴: 표준 라이브러리, 문자열 등
1. 서브시스템 및 서브시스템 DLL, 그리고 네이티브 이미즈를 위한 내부 지원 함수: `Ldr` (이미지 로더), `Csr` (원도우 서브시스템 프로세스 통신 함수), `Rtl` (범용 런타임 라이브러리 루틴), 힙 관리자 등
