...
### `Ntdll.dll`
서브시스템 DLL 또는 네이티브 어플리케이션의 활용을 지원하는 특수 시스템 라이브러리이며, 다음 함수 유형들을 지원한다.

> `Nt` 접두사는 네이티브(native)를 의미하며, 어떠한 [환경 서브시스템](ko.Subsystem.md#환경-서브시스템)에도 종속되지 않은 순수 윈도우 운영체제 코드를 가리킨다.

1. [시스템 서비스](#시스템-서비스) 진입점: `Nt`(네이티브 API)
1. 서브시스템 및 네이티브 이미지 지원 함수: `Ldr`(이미지 로더), `Csr`(원도우 서브시스템 프로세스 통신), `Rtl`(범용 런타임 라이브러리) 등
