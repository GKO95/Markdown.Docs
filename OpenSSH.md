...
위의 절차로부터 SSH 세션은 클라이언트와 서버가 서로의 KEX 비대칭 암호키를 교환하여 **공유 비밀 키 K** 및 **교환 해시 H**가 생성하였다. 다만, 비밀 키 K를 그대로 사용하게 된다면 클라이언트와 서버 모두 공유하고 있기 때문에 SSH 세션 전체가 공격에 취약해진다. 이를 방지하기 위해 공유 비밀 키 K와 교환 해시 H를 조합하여 새로운 키를 파생하되, 클라이언트와 서버가 사용하는 키를 구분시킨다.
